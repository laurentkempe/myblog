...
for (const file of files) {
    ...
    def("FILE", content, { language: "markdown" })
...
`
    ...
    const correctedContent = env.vars.result || env.vars.text
    if (correctedContent && correctedContent.trim() !== content.trim()) {
        await workspace.writeText(file, correctedContent)
