...
As I didn't want to maintain my own worktree management tool, I started researching for other existing ones. That's when I found [Worktrunk](https://worktrunk.dev/) by Maximilian Roos — a CLI for Git worktree management, designed for parallel AI agent workflows written in Rust.
...
I also configured worktrunk hooks; `post-create` to restore nugets with `dotnet restore` and `post-start` to run `dotnet build` so that every new worktree is ready for development. 

`git-wt list` gives you a dashboard of all worktrees with quite some status indicators. When you're done, `git-wt remove` removes worktree; delete branch if merged. And there are much more commands `merge`, `step`...
...
After closing several time by mistake Windows Terminal tabs in which GitHub Copilot CLI agents were running, I realized that the terminal was a weak link in the workflow. If I had multiple worktrees with agents running, I needed a way to keep them all visible and accessible without worrying about accidentally closing one. I remembered about my Linux experience with `tmux`, and I started looking for a Windows equivalent.
...
My second port of Grove a to .NET was done with a squad.
