...
permalink: /2025/03/26/dockerizing-your-dotnet-csharp-mcp-server-for-ai-clients-like-claude-desktop/
date: 3/26/2025 8:23:05 AM
disqusIdentifier: 20250326082305
coverSize: partial
...
draft: true
---
My previous post shown how easy it is to develop a .NET C# MCP server and how to write a client able to communicate with it. Now, the questions is how can we distribute our MCP server to be used by AI clients. In this post, we leverage the knowledge acquired in some of my previous posts to see how to dockerize your .NET C# MCP server.
{/* <!-- more --> */}
# Introduction

Model Context Protocol is on fire 🔥 and it is great to get support for it on .NET through an official C# SDK! 

As I wrote on my previous post, the MCP server is a console application that uses the STDIO transport protocol. And I didn't liked the way I had to specify the path in the MCP Client. So, I searched for another way and thanks to the great post from *Philippe Charrière*, [Creating an MCP Server in Go and Serving it with Docker](https://k33g.hashnode.dev/creating-an-mcp-server-in-go-and-serving-it-with-docker) I thought using a Docker container is a great idea 💡.
...
To configure Claude Desktop to use our MCP server, we need to update the configuration file **%APPDATA%\Claude\claude_desktop_config.json** to the following value:
...
Since some days, we can also use our MCP server with Visual Studio Code Insider. For that we need to add 

![Configure MCP Server in Visual Studio Code](/images/2025/mcp-server-docker-vscode-config.png)

Click on *Edit in settings.json* and add the following lines to the configuration file:
...
Then, we can ask copilot our question and see the answer after accepting to call the tool.
...
Get the source code on GitHub [aiPlayground/MCPServerDocker](https://github.com/laurentkempe/aiPlayground/MCPServerDocker) and if you like it please give it a star ⭐️.
